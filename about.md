...
Measuring your own code is the easy half. I contribute to the projects this work sits on: six changes merged across five projects, two into [llm-d](https://github.com/llm-d/llm-d-router), a Kubernetes-native distributed inference stack, and two into [vLLM](https://github.com/vllm-project/vllm) itself. In llm-d, the first removed a mid-stream latency-prediction path on an open project issue that I claimed with a plan before writing any code; the second added parity coverage between two scorers that disagree when a config is wrong — the reviewer pointed out that my own comment block described a behaviour no test exercised, which was true, so I wrote the test that pins it. The two in vLLM are both in the metrics layer. The others are NVIDIA AIPerf and mistral.rs. A seventh contribution took the other shape: a defect report, with the scorer outputs and the source path that produced them, which another contributor turned into a merged fix. Finding what is wrong and specifying it precisely enough that someone else can fix it is the same skill as fixing it, and often the faster route to a fix. Working inside a codebase you don't own, to the standard its maintainers require, is a different skill from building your own, and the only way to demonstrate it is to do it.
