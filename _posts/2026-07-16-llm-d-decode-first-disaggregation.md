...
Disaggregating *always* is wrong because the KV-cache transfer between prefill and decode workers is not free. In [my 1P1D profiling experiment](/observability/systems-engineering/llm-inference/2026/07/18/vllm-disagg-profiling-1p1d.html), the LMCache/NIXL KV transfer showed up directly as a TTFT cost against an aggregated baseline on the same hardware: in the prefill-heavy regime, mean TTFT nearly tripled (148 → 443 ms) and p99 blew out past a second. For short prompts, you pay the transfer and get little back.
